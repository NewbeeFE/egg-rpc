...
const dispatch = function* (version) {  
  const ctx = this;
  const app = this.app;
  ...
  if (!methodPair || methodPair.length < 2
    || !(rpcMethod = findRpcMethod(app, methodPair, version))
  ) {
...
function findRpcMethod(app, methodPair, version) {
  let rpcClass;
  const rpcMethodName = methodPair[methodPair.length - 1];
  const appPropertyName = version ? ('rpc_' + version) : 'rpc';
  rpcClass = app[appPropertyName];
  console.log(rpcClass);
